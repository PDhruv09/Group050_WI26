...
from src.preprocessing.language import detect_language, normalize_language_label
...
def normalize_records(frame: pd.DataFrame, config: dict, source_dataset: str) -> pd.DataFrame:
    ...
    language_column = first_existing_column(frame.columns, preprocessing.get("language_column_candidates", []))
    ...
    normalized["year"] = normalized["timestamp"].dt.year.astype("Int64")
    normalized["month"] = normalized["timestamp"].dt.month.astype("Int64")
    normalized["year_month"] = normalized["timestamp"].dt.strftime("%Y-%m").astype("string")
    ...
    normalized["language_original"] = frame[language_column].astype("string") if language_column else pd.NA
    normalized["language_detected"] = normalized["prompt_text"].map(detect_language)
    normalized["language"] = (
        normalized["language_original"].map(normalize_language_label)
        if language_column
        else normalized["language_detected"]
    )
    ...
    normalized["is_short_prompt"] = normalized["prompt_word_count"] < int(preprocessing.get("min_prompt_words", 1))
    normalized["is_length_outlier"] = False
...
def filter_records(frame: pd.DataFrame, config: dict) -> pd.DataFrame:
    ...
    if preprocessing.get("remove_short_prompts", True):
        filtered = filtered[~filtered["is_short_prompt"]].copy()

    language_filter = preprocessing.get("language_filter", {})
    if language_filter.get("enabled", False):
        included = {normalize_language_label(value) for value in language_filter.get("include", [])}
        filtered = filtered[filtered["language"].isin(included)].copy()

    if preprocessing.get("drop_duplicates", True):
        filtered = filtered.drop_duplicates(subset=["raw_text_hash"]).copy()

    if preprocessing.get("flag_long_prompts", True) and not filtered.empty:
        quantile = float(preprocessing.get("long_prompt_quantile", 0.99))
        threshold = filtered["prompt_word_count"].quantile(quantile)
        filtered["is_length_outlier"] = filtered["prompt_word_count"] > threshold
        if preprocessing.get("remove_long_prompts", False):
            filtered = filtered[~filtered["is_length_outlier"]].copy()
...
def run_pipeline(input_file: Path, output_file: Path, config: dict, source_dataset: str | None = None) -> dict:
    ...
    return {
        ...
        "split_counts": {
            str(key): int(value) for key, value in processed["data_split"].value_counts(dropna=False).to_dict().items()
        },
        "language_counts": {
            str(key): int(value) for key, value in processed["language"].value_counts(dropna=False).head(20).to_dict().items()
        },
